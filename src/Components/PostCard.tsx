...
export default function PostCard({ data, index }: { data: IRecruitPost, index: number }) {
  ...
  return (
    <div className={`${background(index)}`}>
      {index === 3 ? (
        <div className="flex flex-col w-full h-96 justify-center m-2">
          <p className="font-extrabold text-2xl ml-10  mr-20 break-words">
            프로젝트 진행
          </p>
          <p className="font-extrabold text-2xl ml-10  mr-20 break-words">
            가이드라인
        ...
        <div className="p-3 hover:p-1 box-border cursor-pointer">
